...
pipeline {
    ...
    stages {
        ...
        stage('Write Inventory file') {
           steps {
               script {
                   writeFile(file: "${WORKSPACE}/roles/inventory.yaml", text: "${params.inventory_conf}")
                   sh "ls -l"
               }
           }
        }
    ...
    //     stage('Autodeploy on Lab Nodes') {
    //         when {
    //             // Autodeploy OS only on the TH Medium cluster
    //             expression { params.Nodes == 'Lab_Nodes' }
    //         }		
    //         steps {
    //             ansiblePlaybook(
    //             playbook: '${WORKSPACE}/roles/ST_Autodeploy/main_with_username.yaml',
    //             inventory: '${WORKSPACE}/roles/ST_Autodeploy/inventory.ini',
    //             colorized: true,
    //             extras: '--ssh-extra-args="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"',
    //             extraVars: [
    //                 autodeploy_user: '${Autodeploy_User}',
    //                 autodeploy_pass:[value: '${Autodeploy_Password}', hidden: true],
    //                 autodeploy_target_host: '${Target_Host}',
    //                 autodeploy_os: '${CentOS_version}',
    //                 ansible_password: [value: '${Host_Password}', hidden: true]
    //             ])
    //         }
    //     }

    //     stage('Autodeploy ST_Small') {
    //         when {
    //             // Autodeploy OS only on the TH Small cluster
    //             expression { params.Nodes == 'ST_Small' }
    //         }		
    //         steps {
    //             ansiblePlaybook(
    //             playbook: '${WORKSPACE}/roles/ST_Autodeploy/main_with_username.yaml',
    //             inventory: '${WORKSPACE}/roles/ST_Autodeploy/inventory_TH_Small.ini',
    //             colorized: true,
    //             extras: '--ssh-extra-args="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"',
    //             extraVars: [
    //                 autodeploy_user: '${Autodeploy_User}',
    ...
    //     stage('Autodeploy ST_Medium') {
    //         when {
    //             // Autodeploy OS only on the TH Large cluster
    //             expression { params.Nodes == 'ST_Medium' }
    //         }		
    //         steps {
    //             ansiblePlaybook(
    //             playbook: '${WORKSPACE}/roles/ST_Autodeploy/main_with_username.yaml',
    //             inventory: '${WORKSPACE}/roles/ST_Autodeploy/inventory_TH_Medium.ini',
    //             colorized: true,
    //             extras: '--ssh-extra-args="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"',
    //             extraVars: [
    //                 autodeploy_user: '${Autodeploy_User}',
    ...
    //         when {
    //             // Autodeploy OS only on the TH medium cluster
    //             expression { params.Nodes == 'ST_Large' }
    //         }		
    //         steps {
    //             ansiblePlaybook(
    //             playbook: '${WORKSPACE}/roles/ST_Autodeploy/main_with_username.yaml',
    //             inventory: '${WORKSPACE}/roles/ST_Autodeploy/inventory_TH_Large.ini',
    //             colorized: true,
    //             extras: '--ssh-extra-args="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"',
    //             extraVars: [
    //                 autodeploy_user: '${Autodeploy_User}',
